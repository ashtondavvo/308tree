...
class Tree{
	...
	public:void createTree(){
		...
		createTree(screenWidth/2, screenHeight - 10, -20, (rand() % screenHeight/4) + 40 , vec3(0,1,0));
	}
	...
	public:void createTree(float xPos, float yPos, float zPos, float size, vec3 angle){
		...
		for(int i = 0; i < numOfBranches; i++){
			...
			int newSize = size * (rand() % shrinkMax + shrinkMin) / 1000.0f;
			//recurse
	...
	public:void drawTree(int branches){
		...
		for(int i = 0; i < branches; i++){
			...
			glPushMatrix();
			glTranslatef(xPositions[i]/30, yPositions[i]/30, zPositions[i]/30);
